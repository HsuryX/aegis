...
3. Writes a new entry in the Active Decisions section using the canonical template:
   - `Status` (Draft | Proposed | Accepted | Final | Superseded | Rejected | Deferred | not-applicable | emergency) — mirrors `01-design.md` Decision Lifecycle; that section is canonical. Update this enum when the canonical list changes.
...
- **Status** starts as `Draft` while being written; transitions to `Proposed` when ready for review; becomes `Accepted` after review passes; transitions to `Final` when the Confirmation mechanism demonstrably fires (e.g., the spec's `SC-{n}` test passes, the hook is wired and firing, the audit check returns clean). Terminal states (`Superseded`, `Rejected`, `not-applicable`, `Deferred`) retain the entry for traceability and MUST NOT be cited as `Implements:` trailers on live code
...
- When opening D-13+ project-specific decisions. Common candidates include **Contract Format** (OpenAPI 3.1 / protobuf / AsyncAPI 2.6 / JSON Schema — REQUIRED for cross-trust-boundary interfaces per `02-spec.md`), **Subsystem Ownership** (REQUIRED for standard/large scope with ≥ 3 team members per `AGENTS.md` Multi-Agent Handoff Protocol), **Accessibility Model** (REQUIRED when UI is present — target WCAG level, testing approach, AT support per `standards.md`), deployment model, caching strategy, internationalization, API versioning, deployment safety
