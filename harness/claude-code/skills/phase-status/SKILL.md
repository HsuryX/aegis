...
3. **Lifecycle mode** (`finite-delivery` / `steady-state` from `phase.md`)
4. **Scope classification** (micro / small / standard / large from `audit.md`)
5. **Handoff context** (in-progress work, blockers, next step)
6. **Open decisions** — count and list of titles
7. **Critical gaps** — count and list (blocks phase advancement per the `critical` severity rule)
8. **Non-critical gaps** — count only
9. **Active deviations** — list with expiry conditions; flags any that have outlived their expiry

## Integrity checks

The skill MUST verify all seven sub-checks from the Session Start Protocol in `AGENTS.md` step 3 ("Verify state integrity"):
...
5. Archive files (`decisions-archive.md`, `gaps-archive.md`, `phase-archive.md`) are read when they exist — contradictions between archive and active entries are surfaced to the user before proceeding (see `playbooks/principles.md` Required Behaviors #7 "Archive consultation")
6. SYNC-IMPACT HTML comments at the top of `AGENTS.md` and each `playbooks/*.md` file are checked — if any comment's `date` is after the agent's last-read date, the agent MUST re-read the amended framework file itself before using its rules; if its `downstream_review_required` list includes a file not yet re-read, the agent MUST also re-read that downstream file before proceeding (see `playbooks/principles-gates.md` Sync Impact Reports)
...
Critical gaps: 0
Non-critical gaps: 1
Active deviations: 1 (expires at Phase 2 close)
