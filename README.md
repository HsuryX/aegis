...
| **2. Spec** | Write canonical specifications for the final system | Specifications with conformance criteria |
| **3. Implement** | Write code that faithfully implements specifications | Verified code traced to decisions |

The agent reads `AGENTS.md` at session start (Claude Code reads `CLAUDE.md`, which is a symlink to `AGENTS.md`), which bootstraps the entire workflow.

## Structure

```
AGENTS.md                            # Canonical entry file (RFC 2119 framework)
...
ONBOARDING.md                        # First-time adopter primer for Phase 0
validate.py                          # Mechanical validator (17 checks; see module docstring)
playbooks/                           # Agent-neutral rulebook (read-only for the agent)
  principles.md                      # Tier 0 cross-phase rules (always-load)
  principles-gates.md                # Tier 1 gate-scoped rules (load on gate/amendment)
  principles-conditional.md          # Tier 2 triggered rules (context budget / multi-agent / spirit=letter)
  ...
  identifiers.md                     # Label families (D-design decisions · G-gaps · FR-functional reqs · NFR-non-functional · SC-success criteria · NG-non-goals · L-lessons) + marker rules
...
.agent-state/                        # Agent's working documents
  phase.md                           # Current phase, gate status, session log
  decisions.md                       # Decision ledger + naming table
  gaps.md                            # Gap tracker (critical, non-critical, resolved)
...
harness/                             # Agent-specific adapters (the 5% that varies per agent)
  ...
  cursor/                            # Cursor harness (.cursor/rules/*.mdc)
    .cursor/rules/                   # MDC rule files — thin pointers to playbooks/
      ...
      phase-0.mdc                    # Phase 0 rules (phase-gated activation)
      phase-1.mdc                    # Phase 1 rules
      phase-2.mdc                    # Phase 2 rules
      phase-3.mdc                    # Phase 3 rules
...
**Playbooks** are read-only for the agent. **State files** are the agent's working documents.
...
The bootstrap script copies framework files (preserving the `CLAUDE.md → AGENTS.md` symlink via `cp -a`), resets `.agent-state/` to template-only, prompts for project name + scope + type + terminal phase, writes initial `phase.md`, and runs `validate.py` for a clean-state smoke test.

After bootstrap:

1. `cd /path/to/your-project`
2. Read `ONBOARDING.md` and `AGENTS.md`
3. Begin Phase 0 audit per `playbooks/00-audit.md`

During Phase 1 (when the toolchain is decided), add hooks to `harness/claude-code/settings.json` — see `harness/claude-code/hooks-cookbook.md` Settings Template; replace `<formatter>`, `<linter>`, `<build-command>` with your tools. Optionally configure `.mcp.json` for documentation and search MCP servers. Codex and Cursor users follow the setup checklists in `harness/codex/README.md` and `harness/cursor/README.md`.
...
Then manually reset `.agent-state/` to template-only and edit `phase.md` with your project's scope and type.
...
- **`AGENTS.md` or `CLAUDE.md` collision** — keep the existing file as `AGENTS.local.md` and adopt aegis's as canonical. Record a `framework` gap to reconcile any project-specific rules from the local file into aegis's Amendment Protocol.
...
For projects that produce specifications but no implementation code (governance documents, API contracts), set the terminal phase to `2-spec` in `phase.md`. Phase 3 is marked `not-applicable`.
...
Design decisions are numbered D-1 through D-12 (defined in `playbooks/01-design.md`); project-specific decisions start at D-13. See `playbooks/identifiers.md` for the full label system (`D-`/`G-`/`FR-`/`NFR-`/`SC-`/`NG-`/`L-`).
...
This section is canonical for the symlink mechanics and reader-vs-agent split. For the read/write semantics between framework files (read-only) and project state (read-write), see `ONBOARDING.md § Framework rules vs. project state`.

This README is for humans browsing the repository. `AGENTS.md` is the canonical entry file — it holds the framework itself (Session Start Protocol, Foundational Principle, Quality Primacy, Verdict Discipline, Phase Gates, Workspace Discipline, Authority Discipline, Amendment Protocol, Multi-Agent Handoff Protocol, Tooling Integration). `CLAUDE.md` is a symlink to `AGENTS.md` kept at the repo root so Claude Code continues to find the entry point via its native convention — both names resolve to the same file.
...
- **Codex, Cursor, and other AGENTS.md-aware agents** read `AGENTS.md` directly
